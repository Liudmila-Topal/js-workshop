...
test("Using a callback function with setTimeout", (done) => {
...
test("Using a callback function with setInterval", (done) => {
    // Function to start the countdown with a callback
    function startCountdown(seconds, callback) {
        logger.info(`Starting countdown from ${seconds} seconds...`)
    ...
    startCountdown(5, timeUp); // Countdown from 5 seconds
});
/*
Explanation:
...
test("Adding a student and sorting the list using callbacks", (done) => {
    let students = [
        { name: "Harry Potter", house: "Gryffindor" },
        { name: "Draco Malfoy", house: "Slytherin" },
    ];

    const newStudent = { name: "Cedric Diggory", house: "Hufflepuff" };
